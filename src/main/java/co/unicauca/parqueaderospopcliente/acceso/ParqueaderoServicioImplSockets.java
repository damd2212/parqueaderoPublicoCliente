...
public class ParqueaderoServicioImplSockets implements IParqueadero{
   ...
   //Metodo para consultar una tarifa para carro y devolver el valor total a pagar
    ...
    public List<EstadisticaIngreso> consultarEstadistica(String Fecha,String nitParqueadero)throws Exception{
        ...
        if (jsonEstadistica == null) {
            throw new Exception("No se pudo conectar con el servidor");
        }else{
            if (!jsonEstadistica.equals("NO_ENCONTRADO")) {
                List<EstadisticaIngreso> lista = null;
                lista=parseToEstadisticas(jsonEstadistica);
                return lista;
    ...
    private void parseToParqueadero(Parqueadero par,String Json){
        ...
        par.setPuestosLibres(prop.getProperty("puestosLibres"));
        par.setPuestosOcupados(prop.getProperty("puestosOcupados"));
    }
    ...
    private EstadisticaIngreso parseToEstadistica(String json){
        Gson gson = new Gson();
        EstadisticaIngreso est = new EstadisticaIngreso();
        Properties prop = gson.fromJson(json, Properties.class);
        
        est.setFecha(prop.getProperty("Fecha"));
        est.setHora(prop.getProperty("Hora"));
        est.setConteoTotal(prop.getProperty("ConteoTotal"));
        
        return est;
    }
    
    public List<EstadisticaIngreso> parseToEstadisticas(String Json){
        List<EstadisticaIngreso> objEstadisticas = new ArrayList();
        int i = 1;
        try{
            Gson gson = new Gson();
            Properties prop = gson.fromJson(Json, Properties.class);
            while(true){
                objEstadisticas.add(parseToEstadistica(prop.getProperty(String.valueOf(i))));
                i++;
            }
        }catch(Exception e){
    ...
    public void actualizarIngreso(String NitParqueadero){
        try{
            conectar(IP_SERVIDOR, PUERTO);
            leerFlujoEntradaSalida("actualizarIngreso,"+NitParqueadero);
            cerrarFlujos();
            desconectar();
        }catch(IOException ex){
            Logger.getLogger(ParqueaderoServicioImplSockets.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void actualizarSalida(String NitParqueadero){
        try{
            conectar(IP_SERVIDOR, PUERTO);
            leerFlujoEntradaSalida("actualizarSalida,"+NitParqueadero);
            cerrarFlujos();
            desconectar();
        }catch(IOException ex){
            Logger.getLogger(ParqueaderoServicioImplSockets.class.getName()).log(Level.SEVERE, null, ex);
